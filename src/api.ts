...
const ADMIN_EMAIL_KEY = 'cca_admin_email';
const ADMIN_PASSWORD_KEY = 'cca_admin_password';
...
export function setStoredAdminSession(email: string, password: string): void {
  try {
...
export async function adminLogin(email: string, password: string): Promise<AdminSession> {
  ...
  if (!res.ok) {
    const msg = await res.text().catch(() => '');
    throw new Error(msg || 'Invalid admin email or password');
  }
  const data = (await res.json()) as { email?: string };
  const session = { email: data.email ?? email.trim().toLowerCase(), password };
  setStoredAdminSession(session.email, session.password);
  return session;
}

export async function fetchAdminOverview(session: AdminSession): Promise<AdminOverview> {
  ...
  if (!res.ok) {
    const msg = await res.text().catch(() => '');
    throw new Error(msg || 'Failed to load admin overview');
  }
  return res.json();
}
